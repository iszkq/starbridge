...
async function uploadMatrixMedia(client, file, encrypted = false) {
  ...
  return { uploaded, fileInfo: { v: "v2", key: { alg: "A256CTR", ext: true, k: encodeBase64Url(rawKey), key_ops: ["decrypt"], kty: "oct" }, iv: encodeBase64Url(iv), hashes: { sha256: encodeBase64Url(digest) }, mimetype: file.type || "application/octet-stream", size: file.size } };
}
...
// Send the complete selection as one normal Matrix image event. This keeps a
// row of emoji/stickers together and makes it visible to clients that do not
...
async function sendEmojiImageEvents(client, roomId, items, encryptedRoom, sendFn = client.sendMessage?.bind(client)) {
  ...
  // Never rasterise animated GIF/WebP/APNG into the static row image.  Keep
  // those files as individual Matrix m.image events so other clients can
  // play them too; static selections can still be compacted into one row.
  const animated = item => /(?:gif|webp|apng)$/i.test(String(item?.fileName || item?.name || "")) || /image\/(?:gif|webp|apng)/i.test(String(item?.mimeType || ""));
  const animatedItems = items.filter(animated);
  const staticItems = items.filter(item => !animated(item));
  const batches = [];
  if (staticItems.length) batches.push(staticItems);
  animatedItems.forEach(item => batches.push([item]));
  for (const batch of batches) {
    let file;
    if (batch.length === 1) {
      const blob = await fetchEmojiBlob(batch[0]);
      const type = blob.type || batch[0].mimeType || "application/octet-stream";
      const extension = type.split("/")[1] || String(batch[0].fileName || "img").split(".").pop() || "bin";
      file = new File([blob], batch[0].fileName || `${batch[0].name}.${extension}`, { type });
    } else file = await composeEmojiRowFile(batch);
...
function spaceChildIds(space) {
  if (!space) return [];
  if (typeof space.getSpaceChildren === "function") { const children = space.getSpaceChildren(); if (Array.isArray(children)) return children.map(child => child.roomId || child.room_id || child); }
  const events = space.currentState?.getStateEvents?.("m.space.child") || [];
  return events.filter(event => (event.getContent?.()?.via || []).length > 0).map(event => event.getStateKey?.()).filter(Boolean);
}
...
function eventToMessage(event, room, userId) {
  ...
  const formatted = content.format === "org.matrix.custom.html" && content.formatted_body ? sanitizeFormattedBody(content.formatted_body) : null;
  ...
  return {
    ...
    threadRoot,
    edited: replacement,
    attachment: ["m.image", "m.file", "m.video", "m.audio", "m.sticker"].includes(msgtype) ? { name: content.body, url: content.url || content.file?.url, file: content.file, info: content.info || null, emoji: msgtype === "m.image" && content.body === "表情" && Number(content.info?.w || 0) <= 96 && Number(content.info?.h || 0) <= 96, sticker: msgtype === "m.sticker", type: msgtype === "m.sticker" ? "m.image" : msgtype } : null,
  };
}

async function roomMessages(room, userId, client) {
  const events = room?.getLiveTimeline?.().getEvents?.() || [];
  const redacted = new Set(); const reactions = new Map(); const edits = new Map(); const byId = new Map();
  events.forEach(event => {
    ...
    if (relation?.rel_type === "m.replace" && relation.event_id) { edits.set(relation.event_id, eventContent?.["m.new_content"]?.body || eventContent?.body || ""); return; }
...
function SearchDialog({ client, room, onClose }) {
  ...
  const search = async e => { e.preventDefault(); const term = query.trim(); if (!term) return; setLoading(true); try {
    ...
    setResults(raw.filter(item => !item.result?.room_id || item.result.room_id === room.id));
  } catch (error) {
    try {
...
function Message({ item, client, onReply, onReact, onThread, onEdit, onRedact, onJumpTo, onForward, onMention, onTogglePinMessage, pinnedEventIds = [], selecting, selected, onSelect, grouped = item?.grouped || false }) {
  ...
  const isOfficeAttachment = Boolean(documentAttachment && ["doc", "docx", "xls", "xlsx", "ppt", "pptx"].includes(docKind?.ext));
  useEffect(() => {
...
function Chat({ room, messages, client, typingUsers = [], onLoadMore, onSearch, onSend, onTyping, onReply, onReact, onThread, onEdit, onRedact, onJumpTo, onForward, onEmojiSelect, replyTo, threadRoot, editing, onCancelReply, onCancelThread, onCancelEdit, onUpload, detailsCollapsed, onToggleDetails, selecting, forwardItems, onSelectForward, onStartSelecting, pinnedEventIds, onTogglePinMessage, onOpenDetails, onStartCall }) {
  ...
  React.useEffect(() => { setDraft(editing?.text || ""); setHistoryExhausted(false); if (room?.id) pendingRoomScrollRef.current = room.id; }, [room?.id, editing?.id]);
  ...
  const send = async () => { const text = draft.trim(); if (!text) return; setDraft(""); forceScrollRef.current = true; await onSend(text); setTimeout(() => { const el = scrollRef.current; if (el) el.scrollTo({ top: el.scrollHeight, behavior: "smooth" }); setShowJump(false); }, 350); };
...
function App() {
  ...
  useEffect(() => {
    ...
    const events = room.getLiveTimeline?.().getEvents?.() || [];
    const target = [...events].reverse().find(event => event?.getId?.() && !room.hasPendingEvent?.(event.getId()));
    if (!target) return;
  ...
  const restoreKeys = async ({ type, key, passphrase, version }) => { const crypto = connected?.client?.getCrypto?.(); if (!crypto || !version) return; setCryptoState(s => ({ ...s, restoring: true, restoreProgress: 0, error: null })); try { if (type === "key") { window.orbitRecoveryKeyBytes = key; await crypto.storeSessionBackupPrivateKey(key, version); } if (type === "secret") { if (typeof crypto.loadSessionBackupPrivateKeyFromSecretStorage === "function") { try { await crypto.loadSessionBackupPrivateKeyFromSecretStorage(); } catch (error) { const stored = await crypto.isKeyBackupKeyStored?.(version); if (!stored) throw error; } } } const result = type === "passphrase" ? await crypto.restoreKeyBackupWithPassphrase(passphrase, { progressCallback: p => setCryptoState(s => ({ ...s, restoreProgress: p?.total ? Math.round(((p.successes || 0) / p.total) * 100) : s.restoreProgress })) }) : await crypto.restoreKeyBackup({ progressCallback: p => setCryptoState(s => ({ ...s, restoreProgress: p?.total ? Math.round(((p.successes || 0) / p.total) * 100) : s.restoreProgress })) }); try { await crypto.bootstrapCrossSigning?.({ authUploadDeviceSigningKeys: async () => ({}) }); } catch {} await connected.client.retryImmediatelyDecryption?.(); await refresh(connected.client); setCryptoState(s => ({ ...s, restoring: false, restoreProgress: 100, keyRestored: true })); Toast.success(`密钥恢复完成，导入 ${result?.imported ?? 0} 个会话。若其他客户端仍显示未验证，请完成一次设备验证。`); } catch (error) { const raw = error?.message || "密钥恢复失败"; const mismatch = /does not match|mismatch|match.*decryption key/i.test(raw); const message = mismatch ? `恢复密钥与服务器备份版本 ${version} 不匹配。请确认这是该账号当前 Secret Storage 的恢复密钥；如果备份曾重置，请从 Element“设置 → 安全与隐私”获取最新密钥。` : raw; setCryptoState(s => ({ ...s, restoring: false, error: message })); Toast.error(message); } };
  ...
  const sendEmoji = async item => { try { const response = await fetch(assetRequestUrl(item.url)); if (!response.ok) throw new Error(`表情包下载失败（${response.status}）`); const blob = await normalizeImageBlob(await response.blob()); const file = new File([blob], item.fileName || `${item.name}.gif`, { type: item.mimeType || blob.type || "image/gif" }); const { uploaded, fileInfo } = await uploadMatrixMedia(connected.client, file, Boolean(room.matrixRoom.hasEncryptionStateEvent?.()));
    ...
    const content = { body: item.fileName || item.name, info }; if (fileInfo) content.file = { ...fileInfo, url: uploaded.content_uri }; else content.url = uploaded.content_uri;
    if (typeof connected.client.sendEvent === "function") await connected.client.sendEvent(room.id, "m.sticker", content); else await connected.client.sendMessage(room.id, { msgtype: "m.image", ...content });
