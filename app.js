...
function eventToMessage(event, room, userId) {
  ...
  return {
    ...
    threadRoot,
    edited: replacement,
    attachment: ["m.image", "m.file", "m.video", "m.audio", "m.sticker"].includes(msgtype) ? { name: content.body, url: content.url || content.file?.url, file: content.file, info: content.info || null, emoji: msgtype === "m.image" && content.body === "表情" && Number(content.info?.w || 0) <= 96 && Number(content.info?.h || 0) <= 96, sticker: msgtype === "m.sticker" || content["org.orbit.sticker"] === true, type: msgtype === "m.sticker" ? "m.image" : msgtype } : null,
  };
}
...
function App() {
  ...
  const sendEmoji = async item => { try { const response = await fetch(assetRequestUrl(item.url)); if (!response.ok) throw new Error(`表情包下载失败（${response.status}）`); const blob = await normalizeImageBlob(await response.blob()); const file = new File([blob], item.fileName || `${item.name}.gif`, { type: item.mimeType || blob.type || "image/gif" }); const { uploaded, fileInfo } = await uploadMatrixMedia(connected.client, file, Boolean(room.matrixRoom.hasEncryptionStateEvent?.()));
    ...
    const content = { msgtype: "m.image", body: item.fileName || item.name, info, "org.orbit.sticker": true }; if (fileInfo) content.file = { ...fileInfo, url: uploaded.content_uri }; else content.url = uploaded.content_uri;
