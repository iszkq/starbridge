...
在此目录执行 `./start.ps1`，然后打开 <http://localhost:4174>。脚本会优先使用 Bun，其次使用 Node.js（保留内置 Matrix 反向代理），最后才回退到 Python 静态服务器。要使用内置 Matrix 代理，请使用这个启动脚本；如果直接用 Python 或其他静态服务器，则要求 homeserver 自己配置 CORS。由于浏览器的 ES Module 安全策略，不建议直接双击 `index.html`（`file://`）。

手机或窄屏会自动进入移动端布局。桌面预览可打开 <http://localhost:4174/?mobile=1>；加 `?mobile=0` 可强制桌面布局。

## 换电脑开发

这是一个无需安装前端依赖的原生浏览器项目，克隆仓库后即可继续开发：

1. 安装 [Bun](https://bun.sh/)（推荐）；没有 Bun 时，Node.js 18+ 也可以直接运行代理回退。
...
- 移动端：微信风格单栏会话、按住说话、相册/拍摄/文件、表情面板、左滑多选和底部消息操作。

## 移动端使用

窄屏、触控设备，或地址栏加上 `?mobile=1` 时，会加载 `src/mobile` 单栏布局。桌面三栏保持不变。详细模块说明见 `src/mobile/README.md`。

### 页面结构

- 房间列表、会话、房间详情分成三层全屏页。点会话标题进入详情；在会话空白处向右滑动，或点返回，会回到上一层应用页面，而不会刷新网页。
- 顶部和底部不再使用桌面那条宽横条。退出登录、创建房间放在房间列表和会话右上角的「更多」。
- 别人正在输入时，输入框上方会显示「xxx 正在输入…」。

### 发消息

- 输入框外观按微信：没有加粗等格式工具栏。键盘上的发送/回车是换行，点输入框右侧「发送」才真正发出去。
- 点表情按钮打开底部表情面板，分类用该分类第一个表情表示，可左右滑动看更多分类；表情面板和手机键盘同时只出现一个。
- 点加号从底部选择相册、拍摄或文件。
- 长按输入框进入语音：按住说话，松手发送；上滑到取消区域再松手则不发送。也可以点语音按钮后再按住说话。

### 消息操作

- 向左滑动一条消息进入多选（滑动时消息会跟着走，右侧出现「多选」提示），可继续点选后转发。
- 点消息旁三个点，底部弹出复制、回复、转发、撤回等操作；点空白处收起。三个点不再带表情快捷条，表情回应请用「表情回应」。
- 语音条支持转文字：点语音条下方「转文字」，或从消息菜单选择，识别走 aihubmix 的 whisper-large-v3-turbo，结果会显示在语音条下面。
- 长按消息同样打开操作，适合没有鼠标的环境。
- 点消息下的表情可看是谁回应的；贴纸支持点开大图预览。
...
| 房间通知开关 | `client.setRoomMute(...)` / push rules → `/_matrix/client/v3/pushrules` |
